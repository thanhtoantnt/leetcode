...
export default function ProblemsPage() {
  ...
  return (
    <main>
      <h1 className="mb-2 text-2xl font-bold">LeetCode problems</h1>
      <p className="mb-10 text-sm text-slate-400">
        Each visualized problem is a recorded run of its solution. Every solution lives in{" "}
      ...
      <section>
        <h2 className="mb-4 text-lg font-semibold">
          All solutions · {solutions.reduce((n, s) => n + s.files.length, 0)}
        </h2>
        <div className="space-y-4">
          {solutions.map((s) => (
            <details key={s.pattern} className="rounded-xl border border-[var(--card-border)] bg-[var(--card-bg)] p-4">
              <summary className="cursor-pointer text-sm font-medium">
                {s.pattern.replace(/_/g, " ")} <span className="text-slate-500">({s.files.length})</span>
              </summary>
